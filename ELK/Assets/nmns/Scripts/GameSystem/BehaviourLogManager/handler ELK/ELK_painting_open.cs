...
namespace Cameo
{
    public class ELK_painting_open : IBehaviourLogHandler
    {
        public BehaviourLogBase CreateLog()
        {
            BehaviourLogELK behaviourLog = new BehaviourLogELK();

            behaviourLog.AddElement("strCategory", BehaviourType.painting_open.ToString());
            ...
            string actionLog = string.Format(GetBehaviourReferenceFormat(BehaviourType.painting_open), levelName, missionName, time);
