...
namespace Cameo
{
    public class ELK_mission_completed : IBehaviourLogHandler
    {
        public BehaviourLogBase CreateLog()
        {
            BehaviourLogELK behaviourLog = new BehaviourLogELK();

            behaviourLog.AddElement("strCategory", BehaviourType.mission_completed.ToString());
            ...
            string actionLog = string.Format(GetBehaviourReferenceFormat(BehaviourType.mission_completed), levelName, missionName, time);
