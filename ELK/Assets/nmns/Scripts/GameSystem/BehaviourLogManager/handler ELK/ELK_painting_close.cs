...
namespace Cameo
{
    public class ELK_painting_close : IBehaviourLogHandler
    {
        public BehaviourLogBase CreateLog()
        {
            BehaviourLogELK behaviourLog = new BehaviourLogELK();

            behaviourLog.AddElement("strCategory", BehaviourType.painting_close.ToString());
            ...
            string actionLog = string.Format(GetBehaviourReferenceFormat(BehaviourType.painting_close), levelName, missionName, time);
