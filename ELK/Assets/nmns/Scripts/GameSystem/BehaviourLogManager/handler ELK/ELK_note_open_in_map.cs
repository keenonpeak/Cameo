...
namespace Cameo
{
    public class ELK_note_open_in_map : IBehaviourLogHandler
    {
        public BehaviourLogBase CreateLog()
        {
            BehaviourLogELK behaviourLog = new BehaviourLogELK();

            behaviourLog.AddElement("strCategory", BehaviourType.note_open_in_map.ToString());
            ...
            string actionLog = string.Format(GetBehaviourReferenceFormat(BehaviourType.note_open_in_map), levelName, missionName, time);
