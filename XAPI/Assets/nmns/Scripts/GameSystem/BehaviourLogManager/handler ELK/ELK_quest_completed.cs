...
namespace Cameo
{
    public class ELK_quest_completed : IBehaviourLogHandler
    {
        public BehaviourLogBase CreateLog()
        {
            BehaviourLogELK behaviourLog = new BehaviourLogELK();

            behaviourLog.AddElement("strCategory", BehaviourType.quest_completed.ToString());
            ...
            string actionLog = string.Format(GetBehaviourReferenceFormat(BehaviourType.quest_completed), levelName, missionName, answerContent, time);
